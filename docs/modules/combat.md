...
- **Lokalne odtwarzanie tur i realne działanie w tle (2026-08-08):** Walka jest liczona w całości od razu (`EncounterService::simulate()`), więc historycznie odtwarzanie jej turami przez osobny request Livewire na każdą turę (`nextTurn()`, do ~50 requestów/walkę, aż 16/s przy x5) było czystym marnotrawstwem - a Web Worker Timer opisany wyżej celowo omijał throttling karty w tle, więc te requesty leciały dalej także po zminimalizowaniu okna, godzinami przy farmieniu AFK. Poprzednia wersja tej sekcji dokumentacji zakładała, że w ukrytej karcie system "błyskawicznie kończy walkę (`finishAllTurns()`)" - metoda faktycznie istniała (`MapStub::finishAllTurns()`), ale nigdy nie była wywoływana z JS, więc w praktyce nic nie przyspieszało.
  Naprawione: `dispatchStartPlayback()` (`MapStub.php`) wysyła teraz do JS cały gotowy log tury (`turns`), dane gracza/wroga i max HP/manę w jednym evencie `start-playback`. Dopóki walka nie jest w trybie overlevel (wielu przeciwników - ten zostaje na starej ścieżce `nextTurn()` per-turę), JS odtwarza tury lokalnie: odsłania odpowiedni wpis logu (`data-turn-index`, wcześniej wyrenderowany ukryty), aktualizuje paski HP/many przez `CombatBarFX.setPercent()` i puszcza tę samą animację/dźwięk co wcześniej (`playTurnEffects()`, wydzielone z `Livewire.on('turn-played')`) - **bez żadnego requestu do serwera na turę**. Po ostatniej turze JS woła raz `finishAllTurns()`, co uruchamia istniejący pipeline nagród/level-upów/auto-chaina bez zmian. Gdy karta jest ukryta (`document.hidden`) - na starcie walki lub w trakcie - JS pomija animację i od razu woła `finishAllTurns()`, więc to, co poprzednia wersja dokumentacji opisywała, teraz faktycznie działa.
