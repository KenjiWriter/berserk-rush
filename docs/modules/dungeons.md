...
7. **Licznik Kluczy oraz Multi-Dungeony (Mnożniki Wypraw):**
   ...
   - **Skalowanie Nagród:** Wygrana w wyprawie Multi mnoży przyznawane Złoto, Doświadczenie (XP) oraz liczbę gwarantowanych Skrzyń z Bossa (np. 5x = od 5 do 15 skrzyń) przez wybrany mnożnik (`key_multiplier`) - bez zmian, ten mnożnik nagród nie zależy od tego, czy trudność etapu skaluje się liczbą przeciwników czy statystykami.
   - **Ilość Materiałów/Przedmiotów z Tabeli Zrzutów Bossa (2026-08-06 - redukcja o połowę):** Ilość wylosowana z `min_qty`-`max_qty` * `key_multiplier` była zbyt hojna (np. 24x Wilczy Kieł z pojedynczego x3 Multi-Dungeonu). `DungeonService::calculateStageLoot()` dzieli teraz tę ilość przez 2 (`max(1, round(ilość / 2))`) - dotyczy WYŁĄCZNIE gałęzi `item`/`material` tej tabeli, złoto i klejnoty wylosowane z tego samego rzutu (`reward_type` = `gold`/`gems`) oraz gwarantowane skrzynie z bossa (patrz punkt 6 wyżej) zostają bez zmian.
