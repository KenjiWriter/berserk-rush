...
- **Zaczarowany Magiczny Metal (consumable, `sub_type = 'upgrade_protection'`):** opcjonalny przedmiot chroniący przed karą `downgrade`. Gracz zaznacza jego użycie w Kuźni przed uderzeniem młotem (widoczne wyłącznie dla poziomów `+6` i wyższych, gdzie kara w ogóle obowiązuje); jeśli posiada 1x w ekwipunku, jest zużywany wraz z resztą kosztów **niezależnie od wyniku próby** - w razie porażki przedmiot NIE traci poziomu (materiały i złoto nadal przepadają), w razie sukcesu metal po prostu przepada jako "ubezpieczenie". Sterowane flagą `$useProtectionMetal` w `UpgradeService::upgradeItem()`, przełączaną `Blacksmith::toggleProtectionMetal()`. Próba użycia go na poziomie bez ryzyka regresji (`< +6`) jest blokowana z komunikatem błędu. Wypada z tej samej puli bossów T5 map / lochów D3+ co Kamień Duchowy (patrz `docs/modules/skills.md`), rzadziej.
