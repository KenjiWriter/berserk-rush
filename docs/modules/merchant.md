...
>
> **Pełny asortyment broni (2026-08-07):** Do `ShopEquipmentSeeder::$themes` dopisano brakujące klucze `axe`/`bow`/`wand`/`dagger`/`bell` dla tierów 10-90 (wcześniej te tiery definiowały tylko `sword` - patrz `BalanceMonstersCommand` komentarz przy `$shopTiers`, który już wcześniej zakładał, że sklep sprzedaje 6 typów broni na każdym tierze). Nazwy mirrorują istniejący motyw danego tieru (np. tier 60 "Runiczny Miecz" -> "Runiczny Topór"/"Runiczny Łuk"/"Runiczna Różdżka"/"Runiczny Sztylet"/"Runiczny Dzwon"). Balans jest automatyczny - każdy nowy typ broni używa tej samej bazy statystyk z `$prototypes` (już istniejącej dla tieru 1) przeskalowanej tym samym mnożnikiem `scale` co reszta tieru, więc moc i cena (`ShopService::getBuyPrice()`, zależna wyłącznie od `level_requirement`) są identyczne jak w przypadku miecza z tego samego tieru. Grafiki ikon (`icon` = `Str::slug(nazwa)`) są generowane osobno - patrz `docs/style_guide_item_icons.md` - do czasu wygenerowania plików PNG te pozycje wyświetlają się w sklepie z brakującą ikoną.
