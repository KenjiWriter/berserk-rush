...
> **Uwaga (wzmocnienie leczenia mikstur HP/Many, 2026-08-08):** Efekt natychmiastowego leczenia mikstur `potion-hp-*`/`potion-mana-*`, zużywanych w trakcie **Lochów** (`DungeonService::usePotion()`) oraz **Eventów Lokacji** w trybie Hardcore (`LocationEventService::usePotion()`), został oddzielony od efektu buffu (zwiększenie MAX HP/MP poza walką, pole `hp_pct`/`mana_pct` - **bez zmian**, nadal 5/10/15%) i podniesiony do: Mała (S) = **15%**, Średnia (M) = **35%**, Duża (L) = **50%** maksymalnego HP/Many (pola `heal_pct`/`mana_heal_pct` w `base_stats`). Przy okazji naprawiono lukę, w której mikstury Many nie działały wcale w Lochach (`DungeonService::usePotion()` nie obsługiwał w ogóle many) oraz działały błędnie w Eventach Lokacji (`LocationEventService::usePotion()` sprawdzał nieistniejące pole `mana_amount` zamiast seedowanego `mana_heal_pct`) - obie metody używają teraz spójnie `mana_heal_pct` (procent, analogicznie do `heal_pct`) z fallbackiem na płaskie `mana_amount`.
