                            ...
                            <div class="bg-black/60 rounded-2xl p-1.5 lg:p-2 border border-amber-500/20 shadow-inner">
                                <div class="flex gap-2 justify-center">
                                    @foreach($this->equippedSkills as $cs)
                                        ...
                                        <div class="relative w-9 h-9 sm:w-10 sm:h-10 lg:w-11 lg:h-11 rounded-xl border {{ $cd > 0 ? 'border-slate-700 bg-slate-900' : 'border-amber-500/80 bg-amber-950/80 shadow-[0_0_12px_rgba(245,158,11,0.4)]' }} flex items-center justify-center overflow-hidden transition-transform hover:scale-105" title="{{ $cs->skill->name }}">
                                            @if($cd > 0)
                                            ...
                                            @else
                                                <span class="text-[10px] font-bold text-amber-200 {{ $cd > 0 ? 'opacity-40' : 'opacity-100' }}">{{ mb_substr($cs->skill->name, 0, 3) }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @if(!empty($enemy))
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-2 lg:gap-2.5">
                                    ...
                                    <div class="bg-slate-900/90 border border-blue-800/40 rounded-2xl p-2 lg:p-2.5 xl:p-3 text-center shadow-md">
                                        <div class="text-[11px] sm:text-xs font-semibold text-blue-300 tracking-wider">INT (Wiedza)</div>
                                        <div class="text-sm sm:text-base lg:text-lg xl:text-xl font-black text-blue-100 font-mono">{{ $enemy['stats']['int'] ?? 0 }}</div>
                                    </div>
                                </div>
                            </div>
                        @else
