                ...
                <div class="col-span-1 md:col-span-1 lg:col-span-1 order-2 lg:order-1" id="dungeon-player-panel-container">
                    <div id="dungeon-player-panel"
                        ...
                        <div class="relative p-3 sm:p-4 lg:p-4 xl:p-5 space-y-3 lg:space-y-4">
                            ...
                            {{-- Player Combat Stats --}}
                            @php $dungeonCombatStats = $character->getCombatStats(); @endphp
                            <div class="mt-2.5 pt-2 border-t border-amber-900/40">
                                ...
                                <div class="grid grid-cols-2 gap-1.5 lg:gap-2">
                                    <div class="bg-amber-950/60 border border-yellow-600/40 rounded-xl p-1.5 text-center shadow-md">
                                        ...
                                        <div class="text-xs sm:text-sm font-black text-yellow-300 font-mono">{{ $dungeonCombatStats['crit_chance'] }}%</div>
                                    </div>
                                    <div class="bg-amber-950/60 border border-emerald-600/40 rounded-xl p-1.5 text-center shadow-md">
                                        ...
                                        <div class="text-xs sm:text-sm font-black text-emerald-300 font-mono">{{ $dungeonCombatStats['dodge_chance'] }}%</div>
                                    </div>
                                    <div class="bg-amber-950/60 border border-red-600/40 rounded-xl p-1.5 text-center shadow-md">
                                        ...
                                        <div class="text-xs sm:text-sm font-black text-red-200 font-mono">{{ \App\Helpers\FormatHelper::short($dungeonCombatStats['atk_min']) }} - {{ \App\Helpers\FormatHelper::short($dungeonCombatStats['atk_max']) }}</div>
                                    </div>
                                    <div class="bg-amber-950/60 border border-purple-600/40 rounded-xl p-1.5 text-center shadow-md">
                                        ...
                                        <div class="text-xs sm:text-sm font-black text-purple-200 font-mono">{{ \App\Helpers\FormatHelper::short($dungeonCombatStats['magic_atk_min']) }} - {{ \App\Helpers\FormatHelper::short($dungeonCombatStats['magic_atk_max']) }}</div>
                                    </div>
                                    <div class="col-span-2 bg-amber-950/60 border border-blue-600/40 rounded-xl p-1.5 text-center shadow-md">
                                        ...
                                        <div class="text-xs sm:text-sm font-black text-blue-200 font-mono">{{ \App\Helpers\FormatHelper::short($dungeonCombatStats['defense']) }}</div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <ul class="space-y-2 text-amber-100">
                            ...
                            @foreach($visibleTurns as $index => $turn)
                                <li class="leading-relaxed bg-slate-900/70 border border-red-500/20 rounded-xl px-3 py-2 shadow-sm backdrop-blur-sm text-xs sm:text-sm">
                                    <x-combat-log-entry :turn="$turn" :index="$index" player-name="🛡️ Ty" :enemy-name="'👹 ' . ($monster->name ?? 'Potwór')" />
                                    <span class="text-slate-600 text-xs ml-2 font-mono">[HP: {{ \App\Helpers\FormatHelper::short($turn['playerHp'] ?? 0) }} | Potwór: {{ \App\Helpers\FormatHelper::short($turn['enemyHp'] ?? 0) }}]</span>
                                </li>
                            @endforeach
                            @if($isPlaying)
