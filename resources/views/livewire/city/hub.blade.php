        ...
        <div class="hidden lg:grid grid-cols-12 gap-5 w-full flex-grow auto-rows-[180px] xl:auto-rows-[200px]">
            ...
            <div class="col-span-3 row-span-1 relative group rounded-3xl overflow-hidden border border-amber-900/50 shadow-lg transition-all duration-300 hover:border-amber-500/80 hover:shadow-[0_0_20px_rgba(245,158,11,0.2)]"
                ...
                <button wire:click="goTo('pets')" @click="travelingTo = 'Pety'; $dispatch('play-audio', { type: 'shop' })" @mouseenter="$dispatch('play-audio', { type: 'hover' })" class="w-full h-full flex flex-col items-center justify-center p-4 relative">
                    <div class="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110 opacity-55 mix-blend-luminosity" style="background-image: url('{{ asset('img/pets-bg.png') }}');"></div>
                    <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/60 to-black/30"></div>
                    <div class="relative z-10 text-center">
        ...
        <div class="lg:hidden mt-4 flex flex-col space-y-6 relative z-10">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <button wire:click="goTo('quests')" @click="{{ $gameStage >= 22 ? "travelingTo = 'Tablica Wyzwań'; \$dispatch('play-audio', { type: 'tab' })" : "" }}"
                    class="relative overflow-hidden rounded-2xl border border-teal-800/80 shadow-lg p-6 flex flex-col items-center justify-center min-h-[120px] {{ $character->level < 5 ? 'opacity-60 grayscale' : ($gameStage == 23 ? 'animate-[pulse_1.5s_ease-in-out_infinite] ring-2 ring-amber-500' : '') }}" wire:loading.attr="disabled">
